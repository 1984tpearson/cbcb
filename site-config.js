...
Return ONLY a JSON object: {"summary": "...", "facts": "..."}`,
    ...
    intentions: {
      // ── Promoted from the owner's saved overrides, 2026-09-13 ──────────────
      // maxActive, pacing, pacingReady and sfwNote below were all sitting in
      // the stored config as overrides, diverging from these defaults since
      // 2026-09-07. Every session that read them announced a clash and offered
      // to promote them; none actually did, so the same conversation happened
      // again the next time. They are the defaults now, and the override rows
      // have been deleted, so there is nothing left to diverge. Behaviourally
      // a no-op: the stored values were already the ones in force.
      maxActive: 4,
      ...
      pacing: "\n\nThese are private. Never announce them, never explain them, never let them run the conversation. At most ONE small, natural step toward ONE of them per reply — a question, a hint, an excuse to keep them talking, a suggestion dropped lightly — and only when the conversation offers an opening.",
      ...
      pacingReady: "\n\nThese are private — never announce them or explain what you are doing. But one of them is ready: the groundwork is done, and it is no longer something to work up to. Heavily stear the conversation towards that action and/or perform it if poasible",
